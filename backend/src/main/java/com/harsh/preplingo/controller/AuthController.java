...
public class AuthController {
    ...
    public AuthController(AuthService authService, UserRepository userRepository, JwtService jwtService, UserStreakRepository userStreakRepository, TokenBlacklistService tokenBlacklistService) {
    ...
    public ResponseEntity<AuthResponse> refreshToken(@RequestHeader("Authorization") String refreshToken) {
        if (refreshToken != null && refreshToken.startsWith("Bearer ")) {
            User user = userRepository.findByUsername(jwtService.extractUsername(refreshToken.substring(7))).orElseThrow(() -> new UsernameNotFoundException("User not found"));
            ...
            return ResponseEntity.ok(new AuthResponse(tokens.getAccessToken(), tokens.getRefreshToken(), user, userStreak));
        }
